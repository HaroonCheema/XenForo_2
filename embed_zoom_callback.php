...
$meetingSer = $app->service('FS\EmbedZoomMeeting:Meeting');

// Assume you have the following variables defined
$ZOOM_WEBHOOK_SECRET_TOKEN = $app->options()->fs_webhook_secret_key;
...
if (isset($requestBody['event']) && $requestBody['event'] === 'meeting.started') {

    $accountId = $requestBody['payload']['account_id'] ?? '';
    $meetingId = $requestBody['payload']['object']['id'] ?? '';
    $meetingSer->startMeeting($meetingId);
}
...
if (isset($requestBody['event']) && $requestBody['event'] === 'meeting.participant_left') {
    ...
    $meetingSer->leftMeeting($userName, $participant_uuid, $meetingId);
}
