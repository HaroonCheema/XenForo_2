...
if($(".js-attachmentUploads").find('.progress-bar').length===0)
{
var overallp="<div class='progress-bar'><span class='progress-bar-fill'></span></div>";

$(".js-attachmentUploads").prepend(overallp);	
}
		...
		uploadSuccess: function(file, message, chunk)
		{
			...
			//	console.log(this.getFileRows().length);
...
//console.log('inserting');
		...
		toggleInsertMultiRow: function()
		{
			...
			if ($rows.length > 1)
			{
			var totalf=$rows.length;
		var done=0;	    
$.each($rows, function(key,valueObj){
    
   
    if (typeof $(valueObj).attr('data-attachment-id') !== 'undefined')
{
    done=done+1;
...
var percentagedone=Math.floor((done/totalf)*100);
$(".js-attachmentUploads").find('.progress-bar .progress-bar-fill').text(done+"/"+totalf);
$(".js-attachmentUploads").find('.progress-bar .progress-bar-fill').css('width', percentagedone + '%');
...
//chunked uploads start 

!function(window, document, _undefined)
{
    "use strict";

    XF.AttachmentManagerWithChunkedUploads = XF.extend(XF.AttachmentManager, {
        __backup: {
            'getFlowOptions': '_getFlowOptions'
        },

        getFlowOptions: function()
        {
            var flowOptions = this._getFlowOptions();

            var chunkSize = 10240;
            if (XF.config.chunkedUploadsChunkSize > 0)
            {
                chunkSize = XF.config.chunkedUploadsChunkSize;
            }

            flowOptions.chunkSize = chunkSize * 1024;
            flowOptions.simultaneousUploads = XF.config.chunkedUploadsSimultaneousUploads;
    ...
    XF.Element.register('attachment-manager', 'XF.AttachmentManagerWithChunkedUploads');
}(window, document);
