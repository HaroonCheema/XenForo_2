<?php
// FROM HASH: e201de7948a934849f29cae7105410c0
return array(
...
'main_cell' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__compilerTemp2 = '';
	$__compilerTemp2 .= '
							' . $__templater->func('snippet', array($__vars['thread']['thread_description'], 120, array('stripBbCode' => true, ), ), true) . '
						';
	...
	if ($__templater->method($__vars['xf']['visitor'], 'hasPermission', array('bh_brand_hub', 'bh_canEditDiscussionDesc', ))) {
		$__compilerTemp3 .= '
							<li class="structItem-extraInfoMinor">	
								<a href="' . $__templater->func('link', array('threads/edit-description', $__vars['thread'], ), true) . '" data-xf-click="overlay">' . ($__vars['thread']['thread_description'] ? 'Edit description' : 'Add description') . '</a>
							</li>
						';
	}
	$__compilerTemp3 .= '
	...
	if ($__vars['thread']['discussion_state'] == 'deleted') {
		...
		if (($__vars['thread']['discussion_type'] != 'redirect') AND (($__vars['thread']['reply_count'] >= $__vars['xf']['options']['messagesPerPage']) AND $__vars['xf']['options']['lastPageLinks'])) {
			$__finalCompiled .= '
						...
						';
			$__compilerTemp4 = $__templater->func('last_pages', array($__vars['thread']['reply_count'] + 1, $__vars['xf']['options']['messagesPerPage'], $__vars['xf']['options']['lastPageLinks'], ), false);
			if ($__templater->isTraversable($__compilerTemp4)) {
				foreach ($__compilerTemp4 AS $__vars['p']) {
					$__finalCompiled .= '
