...
var $keycookie="' . $__vars['xf']['options']['xf_cookie_key'] . '";
	var links = document.getElementsByTagName("a");
	for (var li = 0; li < links.length; li++) {
		links[li].addEventListener("click", function(event) {

			var targetElement = event.target || event.srcElement;
			navparent = $(targetElement).parent();
									   console.log(navparent);
			if ($(navparent).hasClass(\'p-navgroup-link--logIn\') || $(navparent).hasClass(\'p-navgroup-link--register\')) {
				return false;
			}

			var bannerlink = "https://amcreviews.com";
...
let randomNumber = Math.floor(Math.random() * 10) + 1;
	 ...
	 {
	 secondwin=0;
	 			var importantStuff = window.open(bannerlink, \'_blank\').focus();					   
	 ...
	 var currenttime=$.now()/1000;
		Cookies.set("' . $__vars['xf']['options']['xf_cookie_key'] . '", currenttime, {path: \'/\' });
...
';
	...
	if ($__vars['xf']['visitor']['user_id']) {
		$__compilerTemp3 .= '
					<a href="' . $__templater->func('link', array('members', $__vars['xf']['visitor'], ), true) . '" class="menu-linkRow">' . 'Your profile' . '</a>
				';
	}
	$__compilerTemp3 .= '
				';
	if ($__vars['xf']['visitor']['user_id']) {
		$__compilerTemp3 .= '
					<a href="' . $__templater->func('link', array('account/account-details', ), true) . '" class="menu-linkRow">' . 'Edit profile' . '</a>
				';
	}
	$__compilerTemp3 .= '
			';
	if (strlen(trim($__compilerTemp3)) > 0) {
		$__compilerTemp1 .= '
	<a class="p-staffBar-link menuTrigger" data-xf-click="menu" data-xf-key="alt+m" role="button" tabindex="0" aria-expanded="false" aria-haspopup="true">' . 'Profile' . '</a>
	<div class="menu" data-menu="menu" aria-hidden="true">
		<div class="menu-content">
			<h4 class="menu-header">' . 'Profile tools' . '</h4>
			' . $__compilerTemp3 . '
		</div>
	</div>
';
...
';
	if ($__vars['xf']['visitor']['user_id']) {
		$__compilerTemp1 .= '
	<a href="' . $__templater->func('link', array('account/bookmarks', ), true) . '" class="p-staffBar-link" >' . 'Favourites' . '</a>
';
	}
	$__compilerTemp1 .= '
			';
...
';
	...
	$__vars['navIcons'] = $__templater->method($__vars['xf']['visitor'], 'getNavIcons', array());
	$__compilerTemp7 = '';
	if ($__vars['navIcons']) {
		$__compilerTemp7 .= '
	<li style="margin-left: ' . $__templater->escape($__vars['xf']['options']['fs_show_nav_icon_space']) . 'px;"></li>
	';
		if ($__templater->isTraversable($__vars['navIcons'])) {
			foreach ($__vars['navIcons'] AS $__vars['key'] => $__vars['value']) {
				$__compilerTemp7 .= '
		<li>
			<a href="' . $__templater->escape($__vars['value']['fs_icon_url']) . '" target="_blank" style="margin: 0px 7px;">
				<img src="' . $__templater->func('base_url', array($__templater->method($__vars['value'], 'getImgUrl', array()), ), true) . '" style="width: ' . $__templater->escape($__vars['xf']['options']['fs_show_nav_icon_dimensions']['width']) . 'px; height: ' . $__templater->escape($__vars['xf']['options']['fs_show_nav_icon_dimensions']['height']) . 'px;" alt="image" />
			</a>
		</li>
	';
			}
	...
	if ($__vars['xf']['visitor']['user_id']) {
		...
		if (($__vars['xf']['visitor']['user_state'] == 'rejected') OR ($__vars['xf']['visitor']['user_state'] == 'disabled')) {
			...
			if (!$__vars['xf']['visitor']['is_banned']) {
				$__compilerTemp8 .= '<a href="' . $__templater->func('link', array('conversations', ), true) . '"
									...
									<div class="menu-footer menu-footer--split">
										<div class="menu-footer-main">
											<ul class="listInline listInline--bullet">
												<li>';
			if (!$__vars['xf']['visitor']['is_banned']) {
				$__compilerTemp8 .= '<a href="' . $__templater->func('link', array('conversations', ), true) . '">' . 'Show all' . '</a>';
			}
			$__compilerTemp8 .= '</li>
												';
