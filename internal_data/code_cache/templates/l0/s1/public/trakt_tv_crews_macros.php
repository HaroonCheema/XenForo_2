<?php
// FROM HASH: 9e1b17d231fd1910f1ba5ebd3d3f0aa8
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<div class="block-row block-row--separated">
		<div class="contentRow movieTab--crews">
			<span class="contentRow-figure contentRow-figure--fixedMedium contentRow-figure--fixedMedium contentRow-figure--fluidWidth">
				<img src="' . ($__templater->escape($__vars['crew']['Person']['image_url']) ?: $__templater->func('base_url', array($__templater->func('property', array('trakt_tv_crewPersonSmallImageUrl', ), false), ), true)) . '" />
			</span>
	...
	if ($__vars['crew']['total_episode_count']) {
		$__finalCompiled .= '
					<div>' . 'trakt_tv_total_x_episodes' . '</div>
				';
	}
	$__finalCompiled .= '
				';
	if ($__vars['crew']['jobs']) {
		...
		if ($__templater->isTraversable($__vars['cast']['jobs'])) {
			foreach ($__vars['cast']['jobs'] AS $__vars['job']) {
				$__finalCompiled .= '
								<li>' . $__templater->escape($__vars['job']['job']) . ' (' . 'trakt_tv_x_episodes' . ')</li>
							';
			}
		}
		$__finalCompiled .= '
