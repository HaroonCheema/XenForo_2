<?php
// FROM HASH: ebab02a8f643577ad48fb4e444e15291
return array(
...
';
		...
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['form']['posid'] ? '' : 'Default Questions') . ' ' . 'Add new question');
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
';
	...
	if ($__vars['question']['hasconditional']) {
		$__compilerTemp2 .= '
				';
		if ($__templater->isTraversable($__vars['question']['hasconditional'])) {
			foreach ($__vars['question']['hasconditional'] AS $__vars['key'] => $__vars['conditional']) {
				$__compilerTemp2 .= '
					<input type="hidden" value="' . $__templater->escape($__vars['conditional']) . '" name="hasconditional[' . $__templater->escape($__vars['key']) . ']" />
				';
			}
		}
		$__compilerTemp2 .= '
	...
	if ($__vars['question']['conditional']) {
		$__compilerTemp3 .= '
				<input type="hidden" value="' . $__templater->escape($__vars['question']['conditional']) . '" name="originalConditional" />
			';
	}
	...
	array(
		...
		'maxlength' => $__templater->func('max_length', array($__vars['question'], 'singleline', ), false),
	), array(
		'label' => 'Placeholder',
