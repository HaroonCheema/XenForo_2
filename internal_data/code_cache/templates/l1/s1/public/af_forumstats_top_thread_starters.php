<?php
// FROM HASH: 7dcfa7cf5cd264b69edc94f28b9b7031
return array(
	...
	if (!$__templater->test($__vars['users'], 'empty', array())) {
		...
		if ($__templater->isTraversable($__vars['users'])) {
			foreach ($__vars['users'] AS $__vars['user']) {
				...
				$__compilerTemp2[] = array(
					'class' => 'dataList-cell--min dataList-cell--alt unit-threads',
					'_type' => 'cell',
					'html' => '
                        <div class="miniCol">
                            ' . $__templater->filter($__vars['threadCounts'][$__vars['user']['user_id']], array(array('number', array()),), true) . '
                        </div>
                    ',
				);
