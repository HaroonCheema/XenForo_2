...
'thread_action_buttons' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		$__finalCompiled .= '
						...
						';
	if ($__templater->func('in_array', array($__vars['thread']['node_id'], $__vars['xf']['options']['fs_filter_node'], ), false) AND $__templater->method($__vars['xf']['visitor'], 'hasPermission', array('fs_latest_thread', 'can_featured_unfea', ))) {
		$__finalCompiled .= '
	';
		$__compilerTemp1 = '';
		if ($__vars['thread']['is_featured']) {
			$__compilerTemp1 .= '
			' . 'Unfeatured' . '
			';
		} else {
		...
		' . $__compilerTemp1 . '
	', array(
			'href' => $__templater->func('link', array('threads/featured', $__vars['thread'], ), false),
			'class' => 'button--link',
			'overlay' => 'true',
		), '', array(
		)) . '
