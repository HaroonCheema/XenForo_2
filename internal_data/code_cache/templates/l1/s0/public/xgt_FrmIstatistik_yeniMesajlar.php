<?php
// FROM HASH: 34be09ccc19370b14a62c8f3abba65d4
return array(
	...
	<ul class="_xgtIstatistik-konuListe">
		...
		if ($__templater->isTraversable($__vars['threads'])) {
			...
			<li class="_xgtIstatistik-satir">
				...
				<div class="_xgtIstatistik-satir--hucre _xgtIstatistik-satir--konu ' . (($__templater->method($__vars['thread'], 'isUnread', array()) AND (!$__vars['forceRead'])) ? ' _yeniIcerik' : '') . '" data-author="' . ($__templater->escape($__vars['thread']['User']['username']) ?: $__templater->escape($__vars['thread']['username'])) . '">	
					';
				...
				if ($__templater->func('property', array('xgt_Frmistatistik_son_yazan', ), false)) {
					...
					if ($__vars['thread']['discussion_type'] == 'redirect') {
						...
						if ($__templater->method($__vars['xf']['visitor'], 'isIgnoring', array($__vars['thread']['last_post_user_id'], ))) {
							...
							$__finalCompiled .= '
								';
							if ($__vars['thread']['LastPoster']) {
								$__finalCompiled .= '
									' . $__templater->func('username_link', array($__vars['thread']['LastPoster'], true, array(
									'defaultname' => $__vars['thread']['LastPoster'],
								))) . '
								';
							} else {
								$__finalCompiled .= '
