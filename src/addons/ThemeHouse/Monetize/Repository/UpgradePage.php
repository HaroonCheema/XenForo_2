...
class UpgradePage extends Repository
{
    ...
    {
        return $this->finder('ThemeHouse\Monetize:UpgradePage')
    ...
    public function suggestUserUpgradePageForUser(
        ...
        foreach ($upgradePages as $upgradePage) {
            ...
            if ($this->enableTimePeriodFilters()) {
                $available = $available->filter(
                    function (\XF\Entity\UserUpgrade $userUpgrade) use ($filter) {
                        return ($filter == 'monthly' && $userUpgrade->length_unit == 'month') || ($filter == 'annual' && $userUpgrade->length_unit == 'year') || $filter == 'all' || $userUpgrade->cost_amount == 0;
                    }
                );
            }
            ...
            $available = $available->filter(
                function (\XF\Entity\UserUpgrade $userUpgrade) {
                    $userCriteria = $this->app()->criteria('XF:User', $userUpgrade->user_criteria);
                    return $userCriteria->isMatched(\XF::visitor());
                }
            );
            ...
            if ($newAvailable || $newPurchased) {
                ...
                if (!$upgradePage->show_all) {
                    ...
                    usort($entities, function ($a, $b) {
                        return $a->display_order >= $b->display_order ? 1 : 0;
                    });
    ...
    public function enableTimePeriodFilters()
    {
        if (!$this->options()->thmonetize_showTimePeriodFilters) {
            return false;
        }

        if (\xf::app()->request()->filter('upgrade_page_id', 'int')) {
            return false;
        }

        $upgradeRepo = $this->repository('XF:UserUpgrade');

        list($available) = $upgradeRepo->getFilteredUserUpgradesForList();

        $monthly = $available->filter(
            function (\XF\Entity\UserUpgrade $userUpgrade) {
                return $userUpgrade->length_unit == 'month';
            }
        );

        $annual = $available->filter(
            function (\XF\Entity\UserUpgrade $userUpgrade) {
                return $userUpgrade->length_unit == 'year';
            }
        );

        return $monthly->count() > 0 && $annual->count() > 0;
    }
