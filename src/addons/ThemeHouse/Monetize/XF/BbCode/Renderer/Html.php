...
class Html extends XFCP_Html
{
    ...
    public function filterString($string, array $options)
    {
        ...
        $inDisabledNode = $options['entity'] instanceof \XF\Entity\Post && in_array($options['entity']->Thread->node_id, \XF::options()->thmonetize_keywordsExcludedNodes);
        $inDisabledContentType = $options['entity'] && in_array($options['entity']->getEntityContentType(), \XF::options()->thmonetize_keywordsExcludedContentTypes);

        if ((!isset($options['thmonetize_stop_keyword_replacement']) || !$options['thmonetize_stop_keyword_replacement']) && !$inDisabledNode && !$inDisabledContentType && !$user->isMemberOf(\XF::options()->thmonetize_keywordsExcludedUsergroups)) {
