...
class UserUpgrade extends XFCP_UserUpgrade
{
    ...
    public function upgradeAddEdit(\XF\Entity\UserUpgrade $upgrade)
    {
        ...
        $userCriteria = $this->app->criteria('XF:User', $upgrade->user_criteria);
        ...
        $reply->setParam('userCriteria', $userCriteria);
    ...
    protected function upgradeSaveProcess(\XF\Entity\UserUpgrade $upgrade)
    {
        ...
        $input = $this->filter([
            ...
            'thmonetize_redirect_url' => 'str',
            'user_criteria' => 'array',
        ]);

        $features = [];
        $featuresData = array_filter($input['thmonetize_features']);
        if (!empty($featuresData)) {
            for ($i = 0; $i < count($featuresData['text']); $i++) {
                if (!$featuresData['text'][$i]) {
                    continue;
                }
                $features[] = [
                    'text' => $featuresData['text'][$i],
                    'description' => $featuresData['description'][$i],
                ];
            }
        }

        $input['thmonetize_style_properties'] = array_filter($input['thmonetize_style_properties']);
        $input['thmonetize_features'] = $features;
