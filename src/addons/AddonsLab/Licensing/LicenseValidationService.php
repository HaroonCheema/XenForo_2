...
class LicenseValidationService
{
    ...
    public function getProductLicenseMessage(
        $licenseKey,
        $productPrefixes,
        $addonName
    )
    {
        $checker = $this->getLicenseChecker();

        $keyPrefix = explode('-', $licenseKey);

        if (!is_array($productPrefixes))
        {
            $productPrefixes = [$productPrefixes];
        }


        if (!isset($keyPrefix[0]) || !in_array($keyPrefix[0], $productPrefixes, true))
        {
            return $this->getInvalidLicenseMessage($addonName);
        }
        ...
        {
            // database exceptions are ignored on front-end and no message is shown
            return '';
        }
        ...
        if ($licenseData !== false)
        {
            ...
            {
                $message = $this->getExpiredTrialMessage($addonName);
            }
            else if ($licenseData->isValid() === false)
            {
                $message = $this->getInvalidLicenseMessage($addonName);
            }
            else if ($licenseData->hasBranding())
            {
                $message = $this->getBrandingMessage($addonName);
            }
        }
        else
        {
            $message = $this->getLicenseEmptyMessage($addonName);
        }
