...
class OwnerPagePost extends AbstractHandler
{
	...
	protected function setupLogEntityContent(ModeratorLog $log, Entity $content)
	{
		...
		$log->content_title = $content->OwnerPage->title ?: '';
		$log->content_url = \XF::app()->router('public')->buildLink('nopath:owner-page-posts', $content);
		$log->discussion_content_type = 'bh_ownerpage';
		$log->discussion_content_id = $content->owner_page_id;
	}

	public function getContentTitle(ModeratorLog $log)
	{
                $ownerPage = \XF::em()->find('XenBulletins\BrandHub:OwnerPage', $log->discussion_content_id);
            
		if ($ownerPage && $log->content_user_id == $ownerPage->user_id)
		{
