...
namespace FS\ReviewsMap\Sitemap;

use XF\Sitemap\BuildState;
use XF\Sitemap\Entry;

use XF\App;
use FS\ReviewsMap\Entity\ReviewmapLog;
use XF\Util\Arr;
...
class Builder
{
	...
	public function buildIndex(ReviewmapLog $sitemap)
	{
		$output = '<?xml version="1.0" encoding="UTF-8"?>' . "\n"
			. '<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">' . "\n";

		$sitemapBase = $this->app->options()->boardUrl . '/reviewsmap.php?c=';
	...
	protected function writeCoreData()
	{
		...
		$extras = Arr::stringToArray($options->reviewmapExtraUrls, '/\r?\n/');
	...
	{
		$options = $this->app->options();
		$autoSubmit = $options->reviewmapAutoSubmit;
		...
		$sitemapUrl = urlencode($options->boardUrl . '/reviewsmap.php');
	...
	/**
	 * @return \FS\ReviewsMap\Repository\ReviewmapLog
	 */
	protected function getSitemapRepo()
	{
		return $this->app->repository('FS\ReviewsMap:ReviewmapLog');
	}
}
