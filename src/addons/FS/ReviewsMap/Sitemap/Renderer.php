...
class Renderer
{
	...
	/**
	 * @var \FS\ReviewsMap\Entity\ReviewmapLog|null
	 */
	protected $sitemap;

	public function __construct(\XF\App $app, \FS\ReviewsMap\Entity\ReviewmapLog $sitemap = null)
	{
		$this->app = $app;
	...
	public function outputSitemap(\XF\Http\Response $response, $counter = 0)
	{
		...
		if ($counter <= 0) {
			if ($sitemap->file_count > 1) {
				...
				$response->setDownloadFileName('reviewmap-index.xml', true);
		...
		if ($fs->has($fileName)) {
			...
			$response->setDownloadFileName(
				$sitemap->file_count > 1 ? 'reviewmap-' . $counter . '.xml' : 'reviewmap.xml',
				true
			);
			...
			$response->httpCode(404);
			$response->contentType('text/plain');
			$response->body('invalid reviewmap file');
		}

		return $response;
	}

	public function buildIndex(\FS\ReviewsMap\Entity\ReviewmapLog $sitemap)
	{
		...
		// failing anyway.
		$sitemapBase = $boardUrl . '/reviewmap-%d.xml';
