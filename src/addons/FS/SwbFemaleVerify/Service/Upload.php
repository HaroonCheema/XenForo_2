...
class Upload extends \XF\Service\AbstractService
{
    ...
    public function __construct(\XF\App $app, \FS\SwbFemaleVerify\Entity\FemaleVerification $verify)
    {
        parent::__construct($app);
        $this->setUser($verify);
    }

    protected function setUser(\FS\SwbFemaleVerify\Entity\FemaleVerification $verify)
    {
    ...
    public function validateImageAsSig($fileName, &$error = null)
    {
        ...
        $fileSizeInKb = intval($options->fs_female_verification_images_size) * 1024;

        $fileSizeInMb = intval($options->fs_female_verification_images_size);

        $megabytes = $filesize / 1024;

        if ($filesize > $fileSizeInKb) {
            ...
            $error = \XF::phrase('fs_female_verification_uploaded_image_file_is_too_big', $params);
    ...
    public function uploadImage($type)
    {
        ...
        foreach ($outputFiles as $code => $file) {
            $dataFile = $this->verify->getAbstractedCustomImgPath($type);
