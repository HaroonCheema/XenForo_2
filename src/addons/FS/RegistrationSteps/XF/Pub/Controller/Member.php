...
class Member extends XFCP_Member
{
    ...
    public function actionAddvouch(ParameterBag $params){
		...
		if (!$visitor->hasPermission('fs_limitations', 'fs_limitations_can_vouch')) {
            ...
            else
            {
                ...
                throw $this->exception($this->notFound(\XF::phrase('fs_l_vouch_not_allowed_please_upgrade',['upgradeUrl'=> $upgradeUrl])));
        }
   ...
   public function actionSavevouch(ParameterBag $params){
    ...
    public function actionMailpopup(ParameterBag $params)
    {
        ...
        $user_group_ids = $visitor['secondary_group_ids'];
        $user_group_ids[] = $visitor['user_group_id'];

        if (in_array(6, $user_group_ids) && (count($user_group_ids) == 1)) {
            throw $this->exception($this->notFound(\XF::phrase('fs_l_appointment_not_allowed_please_upgrade', ['upgradeUrl' => $this->buildLink('account-upgrade/')])));
        }
