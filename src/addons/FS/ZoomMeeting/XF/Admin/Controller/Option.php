...
class Option extends XFCP_Option
{
        ...
        public function actionzoomAuthSetup(ParameterBag $params)
	{
		...
		if ($this->isPost())
		{
			$input = $this->filter([
				...
				'client_secret' => 'str',
                        ...
                        $zoom_client_id = $this->assertOptionExists("zoom_client_id");

                        $zoom_client_id->option_value = $input['client_id'];

                        $zoom_client_id->save();

                        $zoom_client_secret = $this->assertOptionExists("zoom_client_secret");
		...
		return $this->view('XF:Option\ZoomAuthSetupt', 'option_zoom_transport_auth', $viewParams);
	}
}