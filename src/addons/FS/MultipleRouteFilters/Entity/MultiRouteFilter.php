...
namespace FS\MultipleRouteFilters\Entity;
 ...
 */
class MultiRouteFilter extends Entity
{
	...
	protected function rebuildRouteFilterCaches()
	{
		...
		// \XF::runOnce('multiRouteFilterCachesRebuild', function () use ($repo) {
	...
	{
		$structure->table = 'fs_multi_route_filter';
		$structure->shortName = 'FS\MultipleRouteFilters:MultiRouteFilter';
		...
		$structure->columns = [
			...
			'parent_route_filter_id' => ['type' => self::UINT, 'default' => 0]
		];
		...
		$structure->relations = [
			'MainRouteFilter' => [
				'entity' => 'XF:RouteFilter',
				'type' => self::TO_ONE,
				'conditions' => [
					['route_filter_id', '=', '$parent_route_filter_id'],
				],
			]
		];
