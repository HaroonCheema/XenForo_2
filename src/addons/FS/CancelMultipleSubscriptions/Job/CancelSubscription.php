...
class CancelSubscription extends AbstractRebuildJob
{
    ...
    protected function rebuildById($id)
    {
        ...
        if (isset($purchaseRequest['purchase_request_id']) && $purchaseRequest['provider_metadata'] != Null) {
            ...
            $response = $handler->canceledPaymentSubscription($paymentProfile, $subscriptionId);

            if ($response) {
                $purchaseRequest->fastUpdate('is_canceled', 1);
            }
        }
    }
