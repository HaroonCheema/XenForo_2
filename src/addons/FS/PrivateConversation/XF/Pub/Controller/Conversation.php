...
namespace FS\PrivateConversation\XF\Pub\Controller;
...
class Conversation extends XFCP_Conversation
{
    public function actionStartConversation()
    {
        ...
        $quoteMessage = $options->fsPrivateConversationQuoteMessage;

        // get options from Admin CP -> Options -> Start conversation -> New conversation maximum
        $newConversationMaximum = $options->fsPrivateConversationNewConversationMaximum;

        // get options from Admin CP -> Options -> Start conversation -> Exclude usernames
        $excludeUsernames = $options->fsPrivateConversationExcludeUsernames;

        // get options from Admin CP -> Options -> Start conversation -> Exclude user groups
        $excludeUserGroups = $options->fsPrivateConversationExcludeUserGroups;
        ...
        if ($bypass != 'yes') {
            ...
            if ($newConversationCount >= $newConversationMaximum) {
                return $this->error(\XF::phrase('fsPrivateconversation_conversation_limit'));
            }
        } else {
            return $this->noPermission();
        }
        ...
        if ($this->isPost()) {
            ...
            if (!$quoteMessage) {
                // get message
                $message = $message = '[QUOTE]' . \XF::phrase('fsPrivateconversation_in_regards_to_post:') . ' [URL]' . $boardUrl . '/posts/' . $postId . '/[/URL][/QUOTE]';
            }
