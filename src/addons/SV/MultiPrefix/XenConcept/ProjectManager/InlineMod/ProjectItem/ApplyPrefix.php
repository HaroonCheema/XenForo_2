        ...
        if ($return)
        {
            if (isset($options['prefix_id']) && !empty($options['prefix_id']))
            {
                ...
                foreach ($entities AS $entity)
                {
                    ...
                    if ($category->sv_min_project_prefixes)
                    {
                        if (\count($svPrefixIds) < $category->sv_min_project_prefixes)
                        {
                            $entitiesFailedMinRequirement++;
                        }
                    }

                    if ($category->sv_max_project_prefixes)
                    {
                        if (\count($svPrefixIds) > $category->sv_max_project_prefixes)
                        {
                ...
                {
                    $error = \XF::phrase('sv_multiprefix_maximum_prefixes_requirement_for_one_more_projects_has_been_exceeded');
                }
                else if ($entitiesFailedMinRequirement !== 0 && $entitiesFailedMaxRequirement === 0)
                {
                    $error = \XF::phrase('sv_multiprefix_minimum_prefixes_requirement_for_one_more_projects_has_not_been_met');
                }
                else if ($entitiesFailedMinRequirement !== 0 && $entitiesFailedMaxRequirement !== 0)
                {
                    $error = \XF::phrase('sv_multiprefix_minimum_or_maximum_prefixes_requirement_for_one_more_projects_has_not_been_met_excee');
                }
