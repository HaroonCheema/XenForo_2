    ...
    public function canApply(AbstractCollection $entities, array $options = [], &$error = null)
    {
        ...
        if ($return)
        {
            if (isset($options['prefix_id']) && !empty($options['prefix_id']))
            {
                ...
                {
                    $error = \XF::phrase('sv_multiprefix_maximum_prefixes_requirement_for_one_more_products_has_been_exceeded');
                }
                else if ($entitiesFailedMinRequirement !== 0 && $entitiesFailedMaxRequirement === 0)
                {
                    $error = \XF::phrase('sv_multiprefix_minimum_prefixes_requirement_for_one_more_products_has_not_been_met');
                }
                else if ($entitiesFailedMinRequirement !== 0 && $entitiesFailedMaxRequirement !== 0)
                {
                    $error = \XF::phrase('sv_multiprefix_minimum_or_maximum_prefixes_requirement_for_one_more_products_has_not_been_met_excee');
                }
