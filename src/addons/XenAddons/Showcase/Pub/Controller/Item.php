...
class Item extends AbstractController
{
	...
	public function actionAmcReviews(ParameterBag $params)
	{
	    return $this->getAmcThreads($params, 'R', 'fs_reviews', 'showcase/amc-reviews');
	}

	public function actionAmcAlerts(ParameterBag $params)
	{
	    return $this->getAmcThreads($params, 'A', 'fs_alerts', 'showcase/amc-alerts');
	}

	protected function getAmcThreads(ParameterBag $params, $reviewType, $pageSelected, $navLink)
	{
		if (!\XF::visitor()->hasPermission('xa_showcase', 'viewReviewsAlerts'))
		    return $this->noPermission(\XF::phrase('reviews_and_alerts_page_permission'));
	    ...
	    return $this->view('XenAddons\Showcase:Item\Updates', 'xa_sc_item_updates_reviews', $viewParams);
	}
	...
	public function actionLeaveContributorsTeam(ParameterBag $params): AbstractReply
	{
		...
		return $this->view(
			'XenAddons\Showcase:Item\LeaveContributorsTeam',
			'xa_sc_item_leave_contributors_team',
			$viewParams
		);
	}
	...
	public function actionManageContributors(ParameterBag $params): AbstractReply
	{
		...
		return $this->view(
			'XenAddons\Showcase:Item\ManageContributors',
			'xa_sc_item_manage_contributors',
			$viewParams
		);
	}	
	...
	public function actionChangeDates(ParameterBag $params)
	{
		...
		else
		{
			...
			$viewParams = [
				...
				'itemCreateMinute' => $itemCreateMinute,
	
				'itemLastUpdateDate' => $itemLastUpdateDate,
				'itemLastUpdateHour' => $itemLastUpdateHour,
				'itemLastUpdateMinute' => $itemLastUpdateMinute,
	...
	public function actionPublishDraftScheduled(ParameterBag $params)
	{
		...
		if ($this->isPost())
		{
			...
			//$item->create_date_timezone = $publishDateInput['item_timezone']; // TODO maybe add this field like we do in AMS/UBS
	...
	public function actionChangeScheduledPublishDate(ParameterBag $params)
	{
		...
		if ($this->isPost())
		{
			...
			$item->create_date = $publishDate;
			//$item->create_date_timezone = $publishDateInput['article_timezone'];   // TODO maybe add this field like we do in AMS/UBS
			$item->edit_date = \XF::$time;
			$item->last_update = \XF::$time;
		...
		else
		{
			...
			//$itemPublishDate->setTimezone(new \DateTimeZone($item->create_date_timezone)); // TODO maybe add this field like we do in AMS/UBS
			...
			$viewParams = [
				...
				'itemPublishHour' => $itemPublishHour,
				'itemPublishMinute' => $itemPublishMinute,
	...
	// TODO this is currently BETA and there could be some adjustments that need to be made, so this is considered not supported!
	...
	public function actionMarkRead()
	{
		...
		if ($this->isPost())
		{
			...
			return $this->redirect(
				$this->buildLink('showcase'),
				\XF::phrase('xa_sc_all_items_marked_as_read')
			);
		}
		else
		{
