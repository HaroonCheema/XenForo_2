...
namespace XenBulletin\AvatarGallery\Service;

use XenBulletin\AvatarGallery\Entity\AvatarGallery;
use XF\Entity\User;
...
class Upload extends \XF\Service\AbstractService 
{
    ...
    public function __construct(\XF\App $app, AvatarGallery $avatar) {
        parent::__construct($app);
        $this->setUser($avatar);
    }

    protected function setUser(AvatarGallery $avatar) {
...
//        }
    ...
    public function uploadAvatarImage() 
    {
        ...
        if (!$this->avatar->exists()) {
            return $this->throwException(new \LogicException("avatar does not exist, cannot update image"));
        }
        ...
        {
            $dataFile = $this->avatar->getAbstractedCustomAvatartImgPath();
