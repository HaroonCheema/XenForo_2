...
class Setup extends AbstractSetup
{
	...
	public function upgrade1000010Step1()
	{
		...
		if ($tableExists) $sm->renameTable('xf_tv_forum', 'nick97_trakt_tv_forum');
	}
	...
	public function upgrade1000010Step2()
	{
		...
		if ($tableExists)
		{
			...
			$update = ['tv_checked' => 0];
			$db->update('nick97_trakt_tv_thread', $update, 'tv_checked = 1');
		}
	}
	...
	public function upgrade1000010Step4()
	{
		...
		if ($tableExists)
		{
			...
			$sm->alterTable('nick97_trakt_tv_ratings', function (Alter $table) {
	...
	public function upgrade1000010Step5()
	{
		...
		if ($tableExists)
		{
			...
			// MAY NEED TO DROP MOVIE_ID KEY
			$sm->alterTable('nick97_trakt_tv_ratings_node', function (Alter $table) {
	...
	public function upgrade1000010Step6()
	{
		...
		$sm->createTable('nick97_trakt_tv_node', function (Create $table) {
			$table->addColumn('node_id', 'INT', 10);
	...
	public function upgrade1000010Step8()
	{
		...
		$threadChange = $db->fetchAll("SELECT tvthread.*, thread.node_id as node_id, forum.tv_id as mainid, forum.tv_season as tvseason, post.* 
			  FROM nick97_trakt_tv_thread as tvthread
			  LEFT JOIN xf_thread as thread
			  ON (thread.thread_id = tvthread.thread_id)
			  LEFT JOIN nick97_trakt_tv_forum as forum
	...
	public function upgrade1000010Step10()
	{
		...
		$update = ['tv_image' => ''];
		$db->update('nick97_trakt_tv_post', $update, 'tv_image = "/no-poster.png"');
	...
	public function upgrade1000010Step11()
	{
		...
		$threadChange = $db->fetchAll("SELECT thread.*, xfthread.first_post_id 
			FROM nick97_trakt_tv_thread as thread
			LEFT JOIN xf_thread as xfthread
			ON (xfthread.thread_id = thread.thread_id)
	...
	{
		$oldPath = 'src/addons/nick97/TraktTV/defaultdata/LargePosters/no-poster.png';
		$newPath = 'data://tv/LargePosters/no-poster.png';
		File::copyFileToAbstractedPath($oldPath, $newPath);

		$oldPath = 'src/addons/nick97/TraktTV/defaultdata/EpisodePosters/no-poster.png';
		$newPath = 'data://tv/EpisodePosters/no-poster.png';
		File::copyFileToAbstractedPath($oldPath, $newPath);

		$oldPath = 'src/addons/nick97/TraktTV/defaultdata/SmallPosters/no-poster.png';
	...
	public function upgrade2000670Step1()
	{
		...
		foreach ($parentChanges as $parentChange)
		{
			...
			$db->update('nick97_trakt_tv_forum', $update, 'node_id = ?', $parentChange['node_id']);
		}
	}
	...
	public function upgrade2000870Step1()
	{
		$db = $this->db();
		$update = ['tv_checked' => 0];
		$db->update('nick97_trakt_tv_post', $update, 'tv_checked = 1');
	}
	...
	public function upgrade2010170Step3()
	{
		...
		$sm->alterTable('nick97_trakt_tv_forum', function (Alter $table) {
			$table->addColumn('tv_poster', 'TINYINT', 1)->setDefault(0);
		});
	}
	...
	public function upgrade2020000Step6()
	{
		...
		$sm->createTable('nick97_trakt_tv_person', function (Create $table) {
			$table->checkExists(true);
		...
		$sm->createTable('nick97_trakt_tv_cast', function (Create $table) {
			$table->checkExists(true);
		...
		$sm->createTable('nick97_trakt_tv_crew', function (Create $table) {
			$table->checkExists(true);
	...
	public function upgrade2020004Step1()
	{
		...
		$sm->alterTable('nick97_trakt_tv_cast', function (Alter $table) {
		...
		$sm->alterTable('nick97_trakt_tv_crew', function (Alter $table) {
	...
	public function upgrade2020006Step1()
	{
		...
		$sm->alterTable('nick97_trakt_tv_person', function (Alter $table) {
	...
	public function upgrade2020008Step1()
	{
		...
		$sm->alterTable('nick97_trakt_tv_person', function (Alter $table) {
			$table->renameColumn('trakt_person_id', 'imdb_person_id');
		});
	}
	...
	{
		$sm = $this->schemaManager();
		$sm->createTable('nick97_trakt_tv_video', function (Create $table) {
			$table->checkExists(true);
	...
	public function upgrade2020049Step1()
	{
		...
		$sm->createTable('nick97_trakt_tv_company', function (Create $table) {
			$table->addColumn('company_id', 'INT', 10)->primaryKey();
		...
		$sm->createTable('nick97_trakt_tv_network', function (Create $table) {
			$table->addColumn('network_id', 'INT', 10)->primaryKey();
		...
		$sm->alterTable('nick97_trakt_tv_thread', function (Alter $table) {
			...
			$table->addColumn('trakt_network_ids', 'MEDIUMBLOB')->nullable()->setDefault(null)->after('trakt_production_company_ids');
		});
	}
	...
	public function upgrade2020051Step1()
	{
		$sm = $this->schemaManager();
		$sm->alterTable('nick97_trakt_tv_thread', function (Alter $table) {
			$table->addColumn('trakt_last_change_date', 'BIGINT')->setDefault(0)->unsigned(false)->after('trakt_network_ids');
		});
	}

	public function upgrade2020062Step1()
	{
		$this->schemaManager()->alterTable('nick97_trakt_tv_thread', function (Alter $table) {
