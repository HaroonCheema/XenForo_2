...
class Porta121 extends \XF\Import\Importer\AbstractForumImporter
{
	/**
	...
	public function stepArticles(StepState $state, array $stepConfig, $maxTime, $limit = 1000)
	{
		...
		foreach ($articles AS $article)
		{
			...
			$import->bulkSet([
				...
				'article_format' => 1,
				'article_exclude' => $article['article_exclude'],
				'article_sticky' => empty($article['article_sticky']) ? 0 : 1,
	...
	public function stepFeatures(StepState $state, array $stepConfig, $maxTime, $limit = 1000)
	{
		...
		foreach ($features AS $feature)
		{
			...
			$import->bulkSet([
				...
				'feature_media' => empty($feature['feature_media']) ? '' : $feature['feature_media'],
			], ['forceSet' => true]);
			$import->save();
	...
	public function stepAuthors(StepState $state)
	{
		...
		foreach ($authors AS $author)
		{
			...
			$import->bulkSet([
				...
				'author_time' => $author['author_time'],
			], ['forceSet' => true]);
			$import->save();
	...
	public function stepCatlinks(StepState $state, array $stepConfig, $maxTime, $limit = 1000)
	{
		$timer = new \XF\Timer($maxTime);
		
		$catlinks = $this->sourceDb->fetchAll('
			SELECT EWRporta2_catlinks.*
			FROM EWRporta2_catlinks
				INNER JOIN EWRporta2_categories ON (EWRporta2_categories.category_id = EWRporta2_catlinks.category_id)
			WHERE EWRporta2_categories.category_type = "category"
			ORDER BY EWRporta2_catlinks.category_id, EWRporta2_catlinks.thread_id
			LIMIT ?, ?
